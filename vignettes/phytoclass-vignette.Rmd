...
klippy::klippy(position = c("top", "right"), color = "blue")
```
...
The main functions of the package are **Cluster()** and **simulated_annealing()**. 
...
For the examples that follow the argument *niter* equals ten for speed of processing, but should be set higher to obtain convergence. 
...
```{r message=FALSE}
Results <- simulated_annealing(Sm, niter = 10)
```
...
Results <- simulated_annealing(Clust1, niter = 10)
```
...
```{r results-not-default, eval=FALSE}
Results$`condition number`
Results$RMSE
Results$MAE
Results$Error
...
```{r figure-results-not-default, fig.width=7, eval=FALSE}
Results$Figure
```
