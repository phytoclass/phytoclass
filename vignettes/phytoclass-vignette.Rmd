...
For the examples that follow the argument *niter* equals three for processing speed, but should be set much higher to obtain convergence. 
...
```{r message=FALSE}
Results <- simulated_annealing(Sm, niter = 3)
```
...
Results <- simulated_annealing(Clust1, niter = 3)
```
