...
Include helper function in vignette and expand on their help:
...
```{r message=FALSE}
Results <- simulated_annealing(Sm, niter = 20)
```
...
```{r message=FALSE}
Clust1 <- Cluster(Sm,14)$cluster.list[[1]]
# Remove the cluster column/label
Clust1$Clust <- NULL

Results <- simulated_annealing(Clust1, niter = 20)
```
