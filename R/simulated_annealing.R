...
simulated_annealing <- function(S, Fmat = NULL, user_defined_min_max = NULL,
                                do_matrix_checks = TRUE, niter = 500,
                                step = 0.009, weight.upper.bound = 30,
                                verbose = TRUE, seed = NULL,
                                check_converge = 100, alt_pro_name = NULL,
                                preserve_F = TRUE, chemtax_style = TRUE) {
    if (!is.null(seed)) set.seed(seed)
    if (is.null(Fmat)) Fmat <- phytoclass::Fm
    ...
    for (k in 1:niter) {
        if (!verbose) pb$tick()
        Temp <- step^(k)
        Random_neighbour(f_c, Temp, chlv, N = place, place, S,
                         S_weights, minF, maxF)
        num_loop <- ifelse(k > niter - 20, 300, 120)
        Dn <- D <- vector("list", num_loop)
        for (i in seq(num_loop)) {
            temp_rand <- Random_neighbour(f_c, Temp, N = place, chlv,
        ...
        while (length(oob) > 0) {
            ...
            for (i in seq(num_loop)) {
                temp_rand <- Random_neighbour(f_n, Temp, chlv, N, place,
