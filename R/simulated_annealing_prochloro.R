...
simulated_annealing_Prochloro <- function(S, Fmat = NULL,
                                          user_defined_min_max = NULL,
                                          do_matrix_checks = TRUE,
                                          niter = 500, step = 0.009,
    ...
    condition.test <- Condition_test(S[, -ncol(S)], Fmat[, -ncol(Fmat)],
                                     min_max_mat[[1]], min_max_mat[[2]])
    if (verbose) message(paste0("\nCondition number = ",
                                round(condition.test), "\n\n"))
    ...
    # so prochloro's anchor stays at 1 and dimensions line up exactly.
    f_clamp     <- Fmat[, -ncol(Fmat)]
    place_clamp <- which(f_clamp > 0)
    f_clamp[place_clamp] <- pmin(pmax(f_clamp[place_clamp], min_max_mat[[1]]),
                                                            min_max_mat[[2]])
    Fmat[, -ncol(Fmat)] <- f_clamp
} else {
    ...
    f_b <- f_c <- f_n <- nnls_initial[[1]]
    f_b_err <- f_c_err <- f_n_err <- nnls_initial[[2]]
    if (!verbose) {
        pb <- progress::progress_bar$new(
            format = "  Simulated Annealing [:bar] :percent ETA: :eta",
            total = niter, clear = FALSE, width = 60)
    }
    converge_tf <- identical(check_converge, TRUE) ||
                   (is.numeric(check_converge) &&
                    length(check_converge) == 1 &&
                    !is.na(check_converge) && check_converge > 0)
    if (converge_tf) {
        check_converge <- ifelse(isTRUE(check_converge) ||
                                 check_converge >= niter,
                                 niter, check_converge)
        check_converge <- round(seq(1, niter, length.out = check_converge))[-1]
        non_zero_idx   <- which(f_b != 0, arr.ind = TRUE)
        fm_iter <- data.frame(iter   = 0,
                              phyto  = rownames(f_b)[non_zero_idx[, 1]],
                              pigment = colnames(Fmat)[non_zero_idx[, 2]],
                              ratio  = f_b[non_zero_idx])
    }
    W0       <- Prochloro_Wrangling(f_c, min_max_mat[[1]], min_max_mat[[2]])
    minF_fix <- W0[[1]]
    maxF_fix <- W0[[2]]
    step     <- 1 - step
    for (k in 1:niter) {
        ...
        seed_nb <- Prochloro_Random_Neighbour(f_c, Temp, chlv, chlvp,
                                              N = place, place, S, S_weights,
                                              minF_fix, maxF_fix)
        ...
        for (i in seq(num_loop)) {
            chlv  <- f_c[, ncol(f_c)]
            chlvp <- f_c[, ncol(f_c) - 1]
            temp_rand <- Prochloro_Random_Neighbour(f_c, Temp, chlv, chlvp,
                                                    N = place, place, S,
                                                    S_weights, minF_fix, maxF_fix)
            rand_itr[[i]]     <- temp_rand
            rand_itr_err[[i]] <- temp_rand[[2]]
        }
        low_indx      <- which.min(rand_itr_err)
        new_neighbour <- rand_itr[[low_indx]]
        num_loop2     <- ifelse(Temp > 0.3, 10, 2)
        new_neighbour <- Steepest_Descent(new_neighbour[[1]], place, S,
                                          S_weights, num.loops = num_loop2)
        f_n  <- new_neighbour[[1]]
        ...
        while (length(oob) > 0) {
            ...
            for (i in seq(num_loop)) {
                chlv  <- f_c[, ncol(f_c)]
                chlvp <- f_c[, ncol(f_c) - 1]
                temp_rand <- Prochloro_Random_Neighbour(f_n, Temp, chlv, chlvp,
                                                        oob_indx, place, S,
                                                        S_weights,
                                                        minF_fix, maxF_fix)
                rand_itr2[[i]]     <- temp_rand
                rand_itr2_err[[i]] <- temp_rand[[2]]
            }
        ...
        if (verbose) {
            message(paste("Iterations:        ", sprintf("%03d", k),
                          "of", niter,
