...
Prochloro_NNLS_MF_Final <- function (Fn, S, S_Chl, cm, S_dvChl) {
  ...
  b <- crossprod(
    t(Weight_error(Fn, cm)),
    t(Weight_error(S,  cm))
  )
  C_new2 <- t(RcppML::nnls(
    crossprod(t(Weight_error(Fn, cm))),
  ...
  Pb <- crossprod(
    t(Weight_error(Fn[nrow(Fn), 1:(ncol(Fn) - 1)], cm[1:(length(cm) - 1)])),
    t(Weight_error(S[,        1:(ncol(S)  - 1)], cm[1:(length(cm) - 1)]))
  )
  PC_new2 <- t(RcppML::nnls(
    crossprod(t(Weight_error(Fn[nrow(Fn), 1:(ncol(Fn) - 1)], cm[1:(length(cm) - 1)]))),
  ...
  PLE <- tidyr::pivot_longer(
    data = k, cols = cn, names_to = "names", values_to = "vals"
  )
  colorBlindGrey8 <- c("#999999", "#E69F00", "#56B4E9", "#009E73",
                       "#F0E442", "#0072B2", "#D55E00", "#CC79A7",
                       "#009E73", "#001E73", "#013E73")
  gr <- colnames(PLE)[[1]]
  ...
  return(list(
    `F matrix` = Fn, RMSE = error, `condition number` = cd,
