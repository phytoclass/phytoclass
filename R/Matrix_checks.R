...
Matrix_checks <- function(S, Fmat) {
  ...
  message("Removing pigments that occur in less than 1% of samples: ", paste(colnames(s_mat)[s_indx], collapse = ", "))
  ...
  for (i in seq(nrow(check_mat))) {  # skips if no 2s in f_mat_og
    ...
    if (length(pig_col) == 0 && length(phyto_row) > 0) {
      message("Removing phytoplankton group ", check_mat[i, 1], " because pigment ", check_mat[i, 2], " is missing.")
  ...
  message("Removing phytoplankton taxa that map to one pigment or less:", 
          paste(rownames(f_mat)[rowSums(f_mat) <= 1], collapse = ", "))
